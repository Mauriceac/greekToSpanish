...
const author = "Pseudo Dionysius the Areopagite";
const title = "De Caelesti Hierarchia";
...
async function translateText() {
    ...
    if (!fs.existsSync(`./translatedTexts/${title}/chapter_${chapter}`)) {
        fs.mkdirSync(`./translatedTexts/${title}/chapter_${chapter}`, { recursive: true });
    }

    fs.writeFileSync(`./translatedTexts/${title}/chapter_${chapter}/${fileName}`, geminiResponse.response.candidates[0].content.parts[0].text);
